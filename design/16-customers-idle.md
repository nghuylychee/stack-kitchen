...
**Status:** AGREED (2026-09-20 — Tuning pass: khách vào từ cạnh trên + vòng nấu chạy mượt,
backlog #33/#34) *(trước đó BUILT #31)*
...
cho khoảng thời gian không mở màn quán. Bàn quyết định **bao nhiêu khách cùng lúc**, bếp quyết
định **nấu song song được mấy món** — mua lệch một bên thì khách chờ lâu rồi bỏ đi.
...
- Degenerate: **mua 10 bàn, 1 bếp** cho đông khách. Tự phạt: một bếp nấu một món, khách thứ n chờ
  quá `CUS_PATIENCE_MS` là bỏ đi tay không (Rule 7) — thu nhập không tăng mà quán trông thảm.
...
6. **Gọi món.** Ngồi xuống xong, sau `CUS_ORDER_MS` khách gọi **một món ngẫu nhiên trong menu quán**
   (`17-`), hiện bong bóng món trên đầu. Món đã gọi không đổi nữa.
7. **Nấu.** Món cần một **bếp rảnh**; hết bếp thì khách chờ. Vòng tiến trình quanh bong bóng phải
   chạy **mượt theo từng khung hình**, không giật theo nhịp `TICK_MS` (sửa 2026-09-20 (3)): nó là
   **một** animation đặt một lần lúc bắt đầu nấu, dài đúng bằng thời gian nấu, do trình duyệt nội
   suy — vòng đời vẫn chạy theo timer, chỉ có phần vẽ là tách ra. Thời gian nấu
   `COOK_PER_CARD_MS × số nguyên liệu của công thức` (`core/data.ts` `RECIPES`) — món nhiều điểm
   thì lâu hơn và trả nhiều hơn, đó là đánh đổi của `17-`. Tính từ **lúc gọi món**, nếu tổng thời
   gian chờ-và-nấu vượt `CUS_PATIENCE_MS` thì khách bỏ đi tay không: card rung, không gold, không
   XP, bếp (nếu đã chiếm) được trả lại ngay.
8. **Ăn và trả tiền.** Nấu xong → bếp rảnh ngay, card món hiện trên bàn, khách ăn `CUS_EAT_MS` rồi
   trả:
   ...
   ngay, `+N` bay lên như thưởng sau ván. Sau đó khách nhảy về ô cửa rồi biến mất, bàn trống lại.
9. **Thu nhập lúc vắng mặt** (dùng `lastSeen` của `12-` Rule 9): lúc màn quán hiện ra, tính
   ...
   `gold = floor(away_capped / IDLE_CYCLE_MS) × số chỗ ngồi × giá trung bình một khách × IDLE_RATE`,
   `xp` tính cùng cách. "Giá trung bình một khách" lấy theo điểm trung bình của các món **đang có
   trong menu quán**, tip coi như 1.0. Không bàn hoặc menu rỗng → 0.
...
| tip theo loại khách | Local 1.0 (w 5) · Student 0.7 (w 3) · Tourist 1.6 (w 2) | tip 0.5–2.5 | GUESS | ba loại cố định; Tourist hiếm và đáng chờ, Student đông và rẻ |
...
- `design/art.md` "Restaurant meta" §1 (`--cus`), §3 (`public/art/Customer/`), §5 (nhịp nhảy).
...
- Mua thêm bàn → hai khách cùng lúc; mua thêm bếp → hai món nấu song song.
